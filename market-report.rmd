---
title: "R+port"
date: '`r paste0("(GMT+9) ",format(Sys.Date(),tz = "Asia/Seoul",usetz = TRUE))`'
author: Allen park
output:
...
if(weekdays(Sys.Date()) %ni% c("Saturday", "Sunday","Monday") & ("00:00:00" < format(Sys.time(), "%X")) & (format(Sys.time(), "%X") < "09:00:00")==TRUE) {
    DATE = format(Sys.Date()-1,"%Y/%m/%d",tz = "Asia/Seoul",usetz = TRUE)
  } else if(weekdays(Sys.Date())=="Saturday"){
    DATE = format(Sys.Date()-1,"%Y/%m/%d",tz = "Asia/Seoul",usetz = TRUE)
  } else if(weekdays(Sys.Date())=="Sunday"){
    DATE = format(Sys.Date()-2,"%Y/%m/%d",tz = "Asia/Seoul",usetz = TRUE)
  } else if(weekdays(Sys.Date()) == "Monday" & ("00:00:00" < format(Sys.time(), "%X")) & (format(Sys.time(), "%X") < "09:00:00") == TRUE){
    DATE = format(Sys.Date()-3,"%Y/%m/%d",tz = "Asia/Seoul",usetz = TRUE)
  } else {
    DATE = format(Sys.Date(),"%Y/%m/%d",tz = "Asia/Seoul",usetz = TRUE)
  }
...

{
  ...
  for (i in 1:3) dfw[i,8] = ifelse(tail(market_intra(us[i])[[1]],n=1) >=
                                     "15:59:00","CLOSE","OPEN")
  dfw[4,8] = ifelse(tail(market_intra(eu[1])[[1]],n=1) >= "23:34","CLOSE","OPEN")
for (i in 5:6) dfw[i,8] = ifelse(tail(market_intra(eu[i-3])[[1]],n=1) >= "23:29","CLOSE","OPEN")
  for (i in 7:8) dfw[i,8] = ifelse(tail(market_intra(cn[i-6])[[1]],n=1) >=
                                     "14:59","CLOSE","OPEN")
  dfw[9,8] = ifelse(tail(market_intra(as[1])[[1]],n=1)  >= "13:59","CLOSE","OPEN")
  dfw[10,8] = ifelse(tail(market_intra(as[2])[[1]],n=1) >= "13:29","CLOSE","OPEN")
  dfw[11,8] = ifelse(tail(market_intra(ks[1])[[1]],n=1) >= "15:29","CLOSE","OPEN")
...
dfc[1,7] = ifelse(length(market_intra(cmm[1])[[1]]) %in% c("1379","1380"),"CLOSE","OPEN")
dfc[2,7] = ifelse(length(market_intra(cmm[2])[[1]]) %in% c("1375","1376"),"CLOSE","OPEN")
