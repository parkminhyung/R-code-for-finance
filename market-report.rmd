...
row.names(dfw) = NULL
  ...
  for (i in 1:3) dfw[i,8] = ifelse(length(market_intra(us[1])[[1]]) %in% c("389","390"),"CLOSE","OPEN")
dfw[4,8] = ifelse(length(market_intra(eu[1])[[1]]) %in% c("515","516"),"CLOSE","OPEN")
  for (i in 5:6) dfw[i,8] = ifelse(length(market_intra(eu[i-3])[[1]]) %in% c("510","511"),"CLOSE","OPEN")
  for (i in 7:8) dfw[i,8] = ifelse(length(market_intra(cn[i-6])[[1]]) %in% c("240","241"),"CLOSE","OPEN")
  dfw[9,8] = ifelse(length(market_intra(as[1])[[1]]) %in% c("355","356"),"CLOSE","OPEN")
  dfw[10,8] = ifelse(length(market_intra(as[2])[[1]]) %in% c("269","270"),"CLOSE","OPEN")
  dfw[11,8] = ifelse(length(market_intra(ks[1])[[1]]) %in% c("389","390"),"CLOSE","OPEN")
