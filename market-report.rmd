...
w_news = function(market){
  ...
  for (i in 1:4) {
    ...
    url[i] = paste0('http://www.newspim.com/news/lists/?category_cd=107030&page=',i*20-20)
    num = url[i] %>% read_html() %>%
      html_nodes(xpath = '//strong[@class="subject_h"]') %>% grep(pattern = market,x=.)
    
    rurl = url[i] %>% read_html() %>%
      html_nodes(xpath = '//article[@class="thumbgroup"]') %>% .[num] %>%
      html_nodes('a') %>% html_attr('href') %>% .[1]
    
    
    if(length(rurl)!=0 && is.na(rurl)==FALSE){
  ...
  tex[1] = substr(x=tex[1],gregexpr(pattern = '특파원=',text=tex[1])[[1]][1]+4,nchar(tex[1]))
