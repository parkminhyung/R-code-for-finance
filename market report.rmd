...
tex = 'http://m.infostock.co.kr/daily/worldIndex.asp?mode=w&STYPE=CH' %>%
  ...
  html_nodes(xpath = '//div[@class="txtContent"]') %>% .[1] %>% as.character() %>%
  strsplit(x=.,split = "<br><br>") %>% .[[1]] %>%
  .[1:(grep(x=.,"국내증시")[1]-1)]
dat = tex[grep("작성일자",tex)] %>%
  substr(.,gregexpr('작성일자',.)[[1]][1]+7,nchar(.)-10) %>% .[1]
dd = paste0(format(as.Date(dat),"%d"),"일")

tex[1] = tex[1] %>% substr(gregexpr(pattern = dd,text = .)[[1]][1],nchar(.))
if(length(grep(pattern = "<br>",tex[1])) !=0) {
   if(DATE == dat) {
      tex %>% strsplit(x=.,split = '<br>') %>% .[[1]] %>% .[1] %>% cat()
  } else {
    cat("마감시황을 준비하고 있습니다.","\n")
    cat("\n")
  }
